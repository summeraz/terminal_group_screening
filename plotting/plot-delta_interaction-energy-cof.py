...
for i, terminal_group in enumerate(terminal_groups):
    for j, chainlength in enumerate(chainlengths):
        ...
        for job in project.find_jobs({'terminal_group': terminal_group,
            ...
            interaction_energy_lo = job.document['shear_5nN_Etotal']
            interaction_energy_hi = job.document['shear_25nN_Etotal']
            energies.append(interaction_energy_hi[0] - interaction_energy_lo[0])
            all_energies.append(interaction_energy_hi[0] - interaction_energy_lo[0])
...
plt.xlabel('Delta interaction energy, kJ/mol')
ax.set_ylabel('Coefficient of friction')
#plt.xlim([-5000, 0])
plt.ylim([0.09, 0.23])
plt.tight_layout()
plt.savefig('delta-interaction-energy-cof.pdf')
